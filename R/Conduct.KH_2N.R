...
Conduct.KH_2N <- function(string.SpeciesNetwork1, string.SpeciesNetwork2, handle.InputGeneTrees, numeric.NumberOfReps, string.PathDir){
  ...
  for (i in 1:numeric.NumberOfReps){
    ...
    handle.BootStrapped_GeneTreesLikes_SpeciesNetwork1_i <- sample(x = vector.GeneTree_Likelihoods_SpeciesNetwork1, size = numeric.NumberOfGeneTrees, replace = T)
    handle.BootStrapped_GeneTreesLikes_SpeciesNetwork2_i <- sample(x = vector.GeneTree_Likelihoods_SpeciesNetwork2, size = numeric.NumberOfGeneTrees, replace = T)

    #############################################
    # Compute test statistic for BS replicate i #
    #############################################
    numeric.MLE_SpeciesNetwork1_i <- sum(handle.BootStrapped_GeneTreesLikes_SpeciesNetwork1_i)
    numeric.MLE_SpeciesNetwork2_i <- sum(handle.BootStrapped_GeneTreesLikes_SpeciesNetwork2_i)

    numeric.Delta_LnL_BootStrapReplicate_i <- numeric.MLE_SpeciesNetwork1_i - numeric.MLE_SpeciesNetwork2_i
