#' Simulate.GeneTrees_From_SpeciesTree2: function to simulate a set of gene trees given a species tree using HYBRID-LAMBDA under the MSC
#'
...
#' #####################################################
#' Simulate.GeneTrees_From_SpeciesTree2(handle.SpeciesTree = handle.SpeciesTree,
#'                                     string.PathDir = '~/Desktop/',
#'                                     numeric.NumberOfGeneTrees = 10)


#######################################
# Simulate.GeneTrees_From_SpeciesTree2 #
#######################################
Simulate.GeneTrees_From_SpeciesTree2 <- function(handle.SpeciesTree, numeric.NumberOfGeneTrees, string.PathDir){
  ...
  ######################################################
  string.Command_HybridLambda <- "hybrid-Lambda -seed ZZZ -spcu 'XXX' -num YYY -pop 1"
  ...
  ################################
  string.Command_SED <- "sed 's/_1//g' OUT_coal_unit > OUT_coal_unit2"
  ...
  #################################
  handle.SimulatedGeneTrees <- read.tree(paste0(string.Path_To_SimulatedGeneTrees, "2"))
