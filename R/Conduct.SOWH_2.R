#' Conduct.SOWH_2: function to conduct the SOWH_1 STAR test given two distinct species topologies and a set of input gene trees
#'
...
#'################################
#'Conduct.SOWH_2(handle.SpeciesTree_1 = handle.SpeciesTreeX2,
...
# Conduct.SOWH_2 #
#################
Conduct.SOWH_2 <- function(handle.SpeciesTree_1, handle.InputGeneTrees, numeric.NumberOfReps, string.PathDir){
  ...
  for (i in 1:numeric.NumberOfReps){
    ...
    if (Sys.info()["sysname"] == "Darwin"){
      handle.Parametric_Simulated_GeneTrees <- Simulate.GeneTrees_From_SpeciesTree(handle.SpeciesTree = handle.Optimized_SpeciesTopology_1,
